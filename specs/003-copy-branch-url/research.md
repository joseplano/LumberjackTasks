...
**Addendum, 2026-08-17** — this Phase 0 record was written before the query/fragment question was known, and its ordered list is therefore incomplete. Two corrections, both now normative in [contracts/branchUrl.md](./contracts/branchUrl.md) §Stage 1:

1. A sixth step is added, and it splits by URL **component**, not by forge. The **fragment** is discarded on every shape, unconditionally and before the host is matched. The **query string** is discarded only where the branch is placed in a **path** segment, leaving `origin + pathname`; Azure DevOps is exempt from that half because its query is the address (spec FR-003e). The human partner ruled on 2026-08-17 that a query or fragment on a stored repository URL is decoration — it is what a browser address bar yields on the repository page — and does not identify a different repository. The ruling came in two parts: the query half first, scoped to the path shapes; then, once the same failure was verified against the shipped module on Azure DevOps, the fragment half was made universal. Without step 6 the branch lands after the `?` or `#` and the URL opens nothing — as a path appended past a query on GitHub, or as a `?version=GB…` parameter swallowed by a fragment on Azure DevOps — which is what made SC-001 unsatisfiable.
2. Because a query or fragment can *hide* a trailing `.git` or `/` from steps 3 and 4 (`https://host/o/r.git?tab=readme`), those two steps are re-applied to what step 6 leaves. This is what "reduces correctly" was always meant to guarantee: rule 3 removes a `.git` even when the only thing after it is the trailing `/` that rule 4 then takes, so `https://host/owner/repo.git/` and `https://host/owner/repo` are the same address (spec FR-003b). The shipped code and its tests already behaved this way; the earlier wording, read literally, did not say so.
