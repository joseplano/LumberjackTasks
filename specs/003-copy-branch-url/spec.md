...
**Status**: Implemented at `cfaa558` (T001–T013, T015), then amended on 2026-08-17 by two product rulings recorded in §Clarifications. The amendments are **not yet built**: tasks T016–T018 carry them, and T014 (the manual walkthrough) is still open. See [tasks.md](./tasks.md) §Phase 6.
...
- **Repository URL carrying a query string or a fragment on a path-shaped forge** — for example `https://github.com/owner/repo?tab=readme` or `https://gitlab.com/owner/repo#readme`, which is what a browser address bar yields when someone copies the repository page rather than the clone field. The query and the fragment are discarded before the branch path is appended, so the result is `https://github.com/owner/repo/tree/<branch>` (FR-003e). Without this the branch path would land *after* the `?` or `#` and the URL would open nothing while the control still announced `Copy branch URL` — the exact mis-promise FR-006 exists to prevent.
- **Azure DevOps repository URL that already carries a query string** — the version parameter is appended to the existing query rather than replacing it (FR-005a). This forge is deliberately exempt from FR-003e's query rule: its query carries addressing information (`?path=/x`), not decoration.
- **Azure DevOps repository URL carrying a fragment** — the fragment is discarded like anywhere else (FR-003e), so `https://dev.azure.com/org/proj/_git/repo#readme` yields `https://dev.azure.com/org/proj/_git/repo?version=GB<branch>`. Were the fragment kept, the version parameter would be appended *after* it and land inside the fragment (`…#readme?version=GB<branch>`), which a browser never sends as a query — a URL that does not open, announced as `Copy branch URL`.
...
- **FR-003**: Deriving the repository address from the stored value MUST apply, in this order: strip surrounding whitespace; convert the SSH remote form to its https equivalent; strip a trailing `.git`; strip a trailing `/`; discard any fragment; and, wherever the branch will be placed in a path segment, discard any query string as well (FR-003e).
  ...
  - **FR-003e**: A **fragment** carried by the stored value is decoration in every case and MUST always be discarded, on every forge, so that `https://host/owner/repo#readme` addresses the same repository as `https://host/owner/repo`. A **query string** is decoration wherever the branch is placed in a **path** segment — GitHub, GitLab, Bitbucket and the unrecognised-host fallback — and MUST be discarded there too, so that `https://host/owner/repo?tab=readme` likewise addresses the same repository. Where the branch is instead placed in a **query parameter** (Azure DevOps), an existing query string is addressing information: it MUST be preserved and appended to, exactly as FR-005a already requires.
...
- **SC-005**: Every input class listed in the Edge Cases section — empty, blank, SSH shorthand, explicit `ssh://` form, `.git` suffix, trailing slash, surrounding whitespace, non-URL value, unsupported scheme, branch with slashes, a query string or fragment on a path-shaped forge, an existing query string on the query-parameter forge, a fragment on the query-parameter forge — alone, alongside a query, and in the legacy hash-routed domain's form — and each of the four forges plus an unrecognised host — is covered by an automated check, and the whole repository test suite passes.
...
### Session 2026-08-17 *(product ruling by the human partner, resolving a post-implementation contradiction)*

- **Q: When a stored repository URL carries a query string or a fragment, is that decoration to be discarded or addressing information to be preserved?** → A: **decoration, for the path-shaped forges**. The repository address is reduced to its origin and path before a path-shaped branch URL is built, so `https://github.com/o/r?tab=readme` with branch `main` yields `https://github.com/o/r/tree/main`. Azure DevOps is exempt and unchanged: its query genuinely addresses (`?path=/x`), and FR-005a's append-with-`&` behaviour stands. FR-003, FR-003e, §Edge Cases, SC-005.
  - **Why this was asked**: post-implementation `speckit-analyze` found SC-001 ("in 100% of cases... opens that branch") and FR-003 + FR-005a mutually unsatisfiable for this input class. FR-003 never mentioned a query or fragment, so both survived normalisation; FR-005 then appended the branch path *after* them, producing `https://github.com/owner/repo?tab=readme/tree/main`, which opens nothing — while the control still announced `Copy branch URL`, the precise mis-promise FR-006/SC-004 exist to prevent. Measured against the shipped module, not inferred. Evidence: `.specify/bridge/post-implementation-contradiction.md`.
  - **Why this answer**: a query or fragment on a repository URL is browser UI state (`?tab=readme`, `#readme`) that a person picks up by copying the repository page from the address bar; it does not identify a different repository. Discarding it is what the person meant. The rule is scoped to the path shapes precisely because the same is not true of Azure DevOps, where the query is the address.
  - **What this did NOT change**: SC-001 keeps its text and its "100%" unamended — the ruling makes it true rather than narrowing it. FR-005a and the Azure DevOps *query* behaviour are untouched. The three other options considered (preserve and re-assemble the URL; narrow SC-001 and accept the broken output; reject such values so the control copies the branch name) were rejected by the human partner.

- **Q: Does the same problem reach Azure DevOps through a fragment, and if so how is it resolved?** → A: **yes, and the fragment is discarded there too**. Exempting Azure from the discard entirely left `https://dev.azure.com/org/proj/_git/repo#readme` yielding `…/repo#readme?version=GBmain`: FR-005a appends the version parameter to the end of the address, so on a fragment-bearing value it lands *inside* the fragment, and a browser never sends it as a query. The final rule is therefore uniform on fragments and scoped only on queries — a fragment is decoration on every forge; a query is decoration on the path-shaped forges and addressing information on Azure DevOps. Verified against the shipped module before ruling. FR-003e, §Edge Cases, SC-005.
  - **Why this answer**: it is the same principle as the ruling above applied to the other URL component, and it keeps the rule statable in one sentence. The alternatives — assembling the Azure URL by component order so the query precedes the fragment, or naming this an accepted limitation and narrowing SC-001 — were rejected: the first is more code for an outcome still doubtful on the legacy `visualstudio.com` hash-routed domain, and the second reintroduces the control announcing `Copy branch URL` over something that does not open.
