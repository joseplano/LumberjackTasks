...
- **Reports are untouched.** `reports.ts` and `metrics.ts` aggregate over `projectId` with no column filter at all, so FR-019b (reporting numbers must not move) is satisfied without changing a line of them. This is verified, not assumed — see `backend/src/services/reports.ts` and `backend/src/services/metrics.ts`.
